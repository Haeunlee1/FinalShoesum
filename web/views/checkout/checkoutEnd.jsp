...
<body>
	...
	IMP.init('imp68123110'); // 'iamport' 대신 부여받은 "가맹점 식별코드"를 사용
	...
	IMP.request_pay({
	    ...
	    if ( rsp.success ) {
			...
			location.replace('<%=request.getContextPath()%>/mypage/insertOrderedList?userNo=<%=userNo%>&proNo=<%=proNo%>&proCount=<%=proCount%>&cartNo=<%=cartNo%>');	    	
	    } else {
	        var msg = '결제에 실패하였습니다.';
